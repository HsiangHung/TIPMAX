...
![](figures/fig5.png)
The above figure shows the the mocked real-time results. The averaged tips and max tips are the aggregated outputs from the data ingestion from Kafka and batch process from Spark Streaming filtering "current" time over the New York city. The web is auto-flash (with every few seconds). With these processes, one can further consider to compare the features on different locations, e.g. airports or nearby Time Square, etc.
